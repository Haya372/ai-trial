...
以下のいずれかに当てはまる場合、Issueが大きすぎる可能性が高い。

- 受け入れ条件がDB/ドメイン/API/フロントエンドなど複数のレイヤーにまたがる
- 独立してリリース判断できる機能・画面が複数含まれている
- PRDから直接起票された機能Issue（`feature.md` テンプレート）で、まだsub-issueに分割されていない

当てはまる場合は、ここで**ブランチを作成せず** `create-sub-issue` スキルを使って分割することをユーザーに提案する。分割案がユーザーに承認され、sub-issueの起票が完了したら対応完了とする。

当てはまらない場合（すでに分割済みのsub-issue、または元々単一レイヤーで完結する小さいIssue）は次のステップへ進む。
